...
const getDbInstance = () => {
  ...
  if (isSupabaseActive) {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      localDb = globalWithDb._sqliteDb;
    }

    // Seed initial mock data
    const seedDb = (conn: any) => {
      console.log('Seeding initial mock database records...');
      
      // Seed default admin (username: admin, password: password123)
      const passwordHash = crypto.createHash('sha256').update('password123').digest('hex');
      conn.prepare('INSERT OR IGNORE INTO admins (username, password_hash) VALUES (?, ?)')
        .run('admin', passwordHash);

      // Seed bootcamp batches
      const batches = conn.prepare('SELECT COUNT(*) as count FROM bootcamp_batches').get() as { count: number };
      if (batches.count === 0) {
        conn.prepare('INSERT INTO bootcamp_batches (title, next_date, is_active, description) VALUES (?, ?, ?, ?)')
      ...
      const courses = conn.prepare('SELECT COUNT(*) as count FROM recorded_courses').get() as { count: number };
      if (courses.count === 0) {
        const course1Id = conn.prepare('INSERT INTO recorded_courses (title, description, price, thumbnail_path) VALUES (?, ?, ?, ?)')
        ...
        const course2Id = conn.prepare('INSERT INTO recorded_courses (title, description, price, thumbnail_path) VALUES (?, ?, ?, ?)')
        ...
        // Seed video modules
        conn.prepare('INSERT INTO video_modules (course_id, title, secure_video_url, sort_order) VALUES (?, ?, ?, ?)')
      ...
      const images = conn.prepare('SELECT COUNT(*) as count FROM portfolio_images').get() as { count: number };
      if (images.count === 0) {
        conn.prepare('INSERT INTO portfolio_images (file_path, caption, category, display_order) VALUES (?, ?, ?, ?)')
    ...
    const initDb = () => {
      const tableCheck = localDb.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='admins'").get();
      ...
      const settingsCount = localDb.prepare("SELECT COUNT(*) as count FROM site_settings").get() as { count: number };
      if (settingsCount.count === 0) {
        console.log('Seeding initial site settings...');
        const insertSetting = localDb.prepare('INSERT OR IGNORE INTO site_settings (key, value) VALUES (?, ?)');
...
// Export a Proxy that delegates all accesses to the lazily initialized database instance
const db = new Proxy({}, {
  get(target, prop) {
